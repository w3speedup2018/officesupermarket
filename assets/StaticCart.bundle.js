...
function () {
  function StaticCart(section) {
	...
	this.$totalExVat = this.$el.find("[data-cart-ex-vat-total]");
	this.$totalVat = this.$el.find("[data-cart-vat-total]");
  ...
  _createClass(StaticCart, [{
    ...
    value: function _bindEvents() {
	  ...
	  this.$el.on("click.cart-page", ".qty-down", function (event) {	
	    var cartItemRow = $(this).closest("[data-cartitem-id]");			
		var qtyInput = cartItemRow.find('.form-field [data-cartitem-quantity]');
		var newqty = parseInt(qtyInput.val()) - 1;
		qtyInput.val(newqty);
		_this2._editItemQuantity(event);
	  });

	  this.$el.on("click.cart-page", ".qty-up", function (event) {		
		var cartItemRow = $(this).closest("[data-cartitem-id]");
		var qtyInput = cartItemRow.find('.form-field [data-cartitem-quantity]')
		var newqty = parseInt(qtyInput.val()) + 1;
		qtyInput.val(newqty);
    ...
    value: function _editItemQuantity(event) {
      ...
      var quantity = remove ? 0 : parseInt(cartItemRow.querySelector('.form-field [data-cartitem-quantity]').value, 10);
    ...
    value: function _didUpdate(response, thisTimeoutId) {
      ...
      shopify_asyncview__WEBPACK_IMPORTED_MODULE_4__[/* default */ "a"].load('/cart', {
		...
		_this4.$totalExVat.each(function (i, el) {
          el.innerHTML = Shopify.formatMoney( // eslint-disable-line no-param-reassign
          (response.total_price / 1.2), _this4.settings.money_format);
          shopify_currency_converter__WEBPACK_IMPORTED_MODULE_5___default.a.update(el);
        }); // Inject new cart list contents

		_this4.$totalVat.each(function (i, el) {
          el.innerHTML = Shopify.formatMoney( // eslint-disable-line no-param-reassign
          response.total_price - (response.total_price / 1.2) , _this4.settings.money_format);
          shopify_currency_converter__WEBPACK_IMPORTED_MODULE_5___default.a.update(el);
        }); // Inject new cart list contents
